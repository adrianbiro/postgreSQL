...
-- delit kolo milliseconds je jeden byte
SELECT "Bytes", "Milliseconds", CAST("Bytes" AS DECIMAL), "Bytes" / "Milliseconds"
FROM "Track"
-- Ako menit nazov stlpca
SELECT "Name" AS nove_meno FROM "Track"
-- concantenate dve pipe po krajoch a single quotes akoze spojeny string 
SELECT "Name", "Composer" || ' ' || "Composer"  as spojeny_text 
FROM "Track"
-- DISTINCT iba uniq
...
SELECT "Name", "Bytes" 
FROM "Track"
ORDER BY "Bytes" DESC --zostupne

-- zorad podla zostupne ak su rovnake bytes tak podla milliseconds
SELECT "Name", "Bytes", "Milliseconds" 
FROM "Track"
ORDER BY "Bytes" DESC, "Milliseconds"  DESC --pri oboch treba specifikovat 

SELECT "Name", "Bytes", "AlbumId"
FROM "Track"
ORDER BY 
	"AlbumId" DESC, 
	"Bytes" DESC -- poradie moze byt naopak

-- Zoradenie podla odvodenej
--milliseconds na sec a na min. CAST a decimal. Meno pre stlpec
...
SELECT "Name", "Bytes" "AlbumId"
FROM "Track"
WHERE "Bytes" > 5000000 AND "Bytes" < 6000000 -- = jedno iba 
ORDER BY
	"Bytes" DESC
 
SELECT "Name", "Bytes" "AlbumId"
FROM "Track"
WHERE NOT "Bytes" > 5000000 -- OR "Bytes" <= 4000000
...
-- quasi regex Zacina% %konci %v_strede%
SELECT "Name"
FROM "Track"
WHERE "Name" LIKE 'F%' -- kde zacina F
--WHERE "Name" ILIKE 'FA%' -- case insensitive
--WHERE "Name" LIKE '%Shark'
--WHERE "Name" ILIKE '%DE%' --nie je bohvieco

-- limit je specifikum postgres FETCH je SQL standard 
SELECT *
FROM "Track"
LIMIT 10 --kolko riadkov
OFFSET 3 -- preskoc prve 3 a od tade 10
