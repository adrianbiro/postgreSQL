...
SELECT percent_change(100, 98, 5);
SELECT percent_change(100, 50);
SELECT percent_change(NULL, 50);
...
-- PROCEDURE
ALTER TABLE teacher ADD COLUMN personal_days integer;
...
-- LUsing PL/Python to create the trim_county() function
...
-------------------
-- upravene na univerzalnejsiu podobu
CREATE OR REPLACE FUNCTION trim_pattern(input_string text, pattern text )
RETURNS text AS $$
    import re
    cleaned = re.sub(pattern, '', input_string)
    return cleaned
$$
LANGUAGE plpython3u;

SELECT county_name,
       trim_pattern(county_name, ' County')
FROM us_counties_pop_est_2019
ORDER BY state_fips, county_fips
LIMIT 5;
