...
SELECT * FROM "Track"
/* GROUP BY 
 pomaha nam vytvarat zoskupenia a nasledne na ne vieme
aplikovat agregacnu funkciu (count, sum ...)
+++++++++
poradie vykonavanie v postgresql
FROM -> WHERE -> GROUP BY -> HAVING -> SELECT -> DISTINCT -> ORDER BY -> LIMIT
++++++++
Samotne group by bez agregacnej funkcie funguje ako 
DISTINCT a vrati unikatne hodnoty lebo na pozadi sa ako keby
vytvoria tieto skupinky  
*/
SELECT "GenreId"
FROM "Track"
GROUP BY "GenreId" -- da len unikatne GenereID
/* ked sa pouzije agregatna funkcia v SELECT musi obsahovat to čo 
sa ide v GROUP BY agregovať. Hviezda je velke nono */
SELECT * -- toto nerob
FROM "Track"
GROUP BY "GenreId"

-- Co ked chceme pre kazdy zaner pocet skladieb v DB 
SELECT "GenreId", COUNT(*) -- suhrnny pocet pre jazdy zaner
-- SELECT COUNT(*) -- tu netreba dat konkretny select, ale je to prehladnejsie 
FROM "Track"
GROUP BY "GenreId"  -- zoradit podla

/*to iste ale iba  s obmedzenim, ked je b > ...*/
...
/*zacat SELECTOM a FROM; potom premenit na 
SUM a GROUP BY; nakoniec ORDER BY SUM() a LIMIT */
SELECT "AlbumId", SUM("UnitPrice") AS final_price
FROM "Track"
GROUP BY "AlbumId" --column "Track.AlbumId" must appear in the GROUP BY clause or be used in an aggregate function
ORDER BY final_price DESC --zoradit podla nie GROUP
LIMIT 2
...
-- Vyfiltruj id albumov, ktorych suma cien skladieb je vacsiaako 8
SELECT "AlbumId", SUM("UnitPrice") AS final_price
FROM "Track"
GROUP BY "AlbumId"
HAVING SUM("UnitPrice") > 8 /* Agregacna funkciaide iba pri GROP BY nie vo WHERE, 
 HAVING je v poradi vykonanie za GROUP BY tak toto  funguje
 preto znove robim SUM*/
ORDER BY final_price DESC
LIMIT 2
/*
poradie vykonavanie v postgresql
...
ORDER BY total DESC;
---
/*Na poradi GROUP BY zalezi*/
SELECT "GenreId", "AlbumId", count(*)
FROM "Track"
--GROUP BY "GenreId", "AlbumId" --toto da ina poradie ako to dole
GROUP BY "AlbumId", "GenreId" 
